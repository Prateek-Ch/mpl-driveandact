...
class I3D(torch.nn.Module):
    def __init__(self, config_file, architecture_config):
        ...
        modality = architecture_config["modality"]
        dropout_prob = architecture_config["dropout_prob"]
        name = architecture_config["name"]
...
class I3DLogit(torch.nn.Module):
    def __init__(self, config_file, architecture_config):
        super(I3DLogit, self).__init__()
        num_classes = config_file["train"]["num_classes"]
        modality = architecture_config["modality"]
        dropout_prob = architecture_config["dropout_prob"]
        name = architecture_config["name"]
        ...
        # Initial Layers
        self.conv3d_1a_7x7 = Unit3Dpy(out_channels=64, in_channels=in_channels,
                                     kernel_size=(7, 7, 7), stride=(1, 2, 2), padding='SAME')
        self.maxPool3d_2a_3x3 = MaxPool3dTFPadding(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        self.conv3d_2b_1x1 = Unit3Dpy(out_channels=64, in_channels=64,
                                     kernel_size=(1, 1, 1), padding='SAME')
        self.conv3d_2c_3x3 = Unit3Dpy(out_channels=192, in_channels=64,
        ...
        self.maxPool3d_4a_3x3 = MaxPool3dTFPadding(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        ...
        self.maxPool3d_5a_2x2 = MaxPool3dTFPadding(kernel_size=(1, 2, 2), stride=(1, 2, 2), padding='SAME')

        self.mixed_5b = Mixed(832, [256, 160, 320, 32, 128, 128])
        self.mixed_5c = Mixed(832, [384, 192, 384, 48, 128, 128])

        # Frame-wise Detection Head
        self.avg_pool = torch.nn.AvgPool3d((1, 7, 7), (1, 1, 1))  # Keep temporal dimension (T)
        self.dropout = torch.nn.Dropout(dropout_prob)
        self.conv3d_0c_1x1 = Unit3Dpy(in_channels=1024, out_channels=self.num_classes,
                                     kernel_size=(1, 1, 1), activation=None,
    ...
    def forward(self, inp):
        ...
        # Preserve temporal axis
        out = self.avg_pool(out)  # Shape: (B, C, T, 1, 1)
        out = self.dropout(out)
        out = self.conv3d_0c_1x1(out)  # Shape: (B, num_classes, T, 1, 1)

        out = out.squeeze(3).squeeze(3)  # Shape: (B, num_classes, T)
        out = out.permute(0, 2, 1)      # Shape: (B, T, num_classes)

        return out  # Output for each frame in the sequence
